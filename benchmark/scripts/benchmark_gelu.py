...
def bench_speed_gelu(input: SingleBenchmarkRunInput) -> SingleBenchmarkRunOutput:
    ...
    if provider == "liger":
        ...
        raise ValueError(f"Invalid provider: {provider} for GELU")
...
def bench_memory_gelu(input: SingleBenchmarkRunInput) -> SingleBenchmarkRunOutput:
    seq_len = input.x
    bsz = input.extra_benchmark_config["bsz"]
    hidden_size = input.extra_benchmark_config["hidden_size"]
    intermediate_size = input.extra_benchmark_config["intermediate_size"]
...
if __name__ == "__main__":
    ...
    run_benchmarks(
        bench_test_fn=bench_speed_gelu,
    ...
    run_benchmarks(
        bench_test_fn=bench_memory_gelu,
        kernel_operation_modes=["full", "forward", "backward"],
        metric_name="memory",
        metric_unit="MB",
        **common_configs,
